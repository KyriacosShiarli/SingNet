...
class convVAE(object):
	def __init__(self,dim_z,x_train,x_test):
		...
		self.layer1 = one_d_conv_layer_fast(self.inpt,self.in_filters[0],1,self.filter_lengths[0],activation = T.nnet.softplus,param_names = ["W1",'b1'],pool=5.) 
		self.params+=self.layer1.params
		self.layer2 = one_d_conv_layer_fast(self.layer1.output,self.in_filters[0],self.in_filters[0],self.filter_lengths[0],activation = T.nnet.softplus,param_names = ["W1",'b1'],pool=5.) 
