...
class convVAE(object):
	def __init__(self,dim_z,x_train,x_test):
		...
		self.in_filters = [15,15,15]
		self.filter_lengths = [30.,30.,30.]
		self.params = []
		#magic = 73888.
		magic = 39600.
		####################################### LAYERS ######################################

		self.layer1_fast = one_d_conv_layer_fast(self.inpt,self.in_filters[0],1,self.filter_lengths[0],activation = T.nnet.softplus,param_names = ["W1",'b1'],pool=5.) 
		...
		self.layer2_fast = one_d_conv_layer_fast(self.layer1_fast.output,self.in_filters[0],self.in_filters[0],self.filter_lengths[0],activation = T.nnet.softplus,param_names = ["W1",'b1'],pool=5.) 
		...
		self.test_fast = T.flatten(self.layer2_fast.output,outdim = 2)
		self.latent_layer_fast = variational_gauss_layer(self.test_fast,magic,dim_z)
		self.hid_layer_fast = hidden_layer(self.latent_layer_fast.output,dim_z,magic)
		self.hid_out_fast = self.hid_layer_fast.output.reshape((self.inpt.shape[0],self.in_filters[-1],1,int(magic/self.in_filters[-1])))
		self.deconv1_fast = one_d_deconv_layer_fast(self.hid_out_fast,self.in_filters[2],self.in_filters[2],self.filter_lengths[2],pool=5.,param_names = ["W3",'b3'],activation=T.nnet.softplus,distribution=False)
		self.deconv2_fast = one_d_deconv_layer_fast(self.deconv1_fast.output,self.in_filters[2],self.in_filters[2],self.filter_lengths[2],pool=5.,param_names = ["W3",'b3'],activation=T.nnet.softplus,distribution=True)
		...
		self.test_fast_layer1 = theano.function([self.inpt],self.layer1_fast.output)
		self.test_fast_layer2 = theano.function([self.inpt],self.layer2_fast.output)
		self.test_fast_flat= theano.function([self.inpt],self.test_fast)
		self.test_fast_lat= theano.function([self.inpt],self.latent_layer_fast.output)
		self.test_fast_deconv1= theano.function([self.inpt],self.deconv1_fast.output)
		self.test_fast_deconv2= theano.function([self.inpt],self.deconv2_fast.output)
		...
		self.deconvolve1 = theano.function([self.layer1.inpt],self.deconv1.output)
...
if __name__ == "__main__":
	...
	# tic2 = time.time()
	tic = time.time()
	out = net.test_fast_deconv2(net.x_train)
	toc =time.time()
	tic2 = time.time()
