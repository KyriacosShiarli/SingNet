...
from layers_cuda import one_d_conv_layer_cuda,one_d_deconv_layer_cuda
...
class convVAE(object):
	def __init__(self,dim_z,x_train,x_test):
		...
		self.layer1 = one_d_conv_layer_cuda(self.inpt,self.in_filters[0],1,self.filter_lengths[0],activation = T.nnet.softplus,param_names = ["W1",'b1'],pool=5.) 
		self.params+=self.layer1.params
		self.layer2 = one_d_conv_layer_cuda(self.layer1.output,self.in_filters[0],self.in_filters[0],self.filter_lengths[0],activation = T.nnet.softplus,param_names = ["W1",'b1'],pool=5.) 
		...
		self.deconv1 = one_d_deconv_layer_cuda(self.hid_out,self.in_filters[2],self.in_filters[2],self.filter_lengths[2],pool=5.,param_names = ["W3",'b3'],activation=T.nnet.softplus,distribution=False)
		self.params+=self.deconv1.params
		self.deconv2 = one_d_deconv_layer_cuda(self.deconv1.output,1,self.in_filters[2],self.filter_lengths[2],pool=5.,param_names = ["W4",'b4'],activation=None,distribution=True)
