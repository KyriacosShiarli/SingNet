...
from theano.sandbox.cuda.dnn import dnn_pool as pool_2d
...
class convVAE(object):
	def __init__(self,dim_z,x_train,x_test,diff=None,magic=5000):
		...
		# LAYER 2 ##############################
		self.conv2 = one_d_conv_layer(self.layer1_out,self.in_filters[1],self.in_filters[0],self.filter_lengths[1],param_names = ["W2",'b2']) 
		self.params+=self.conv2.params
		self.bn2 = batchnorm(self.conv2.output)
		self.nl2 = self.activation(self.bn2.X)
		self.maxpool2 = pool_2d(self.nl2,[3,1],stride=[2,1], mode='average_exc_pad').astype(theano.config.floatX)
		...
		# Deconv 2 ######################
		self.deconv2 = one_d_deconv_layer(self.deconv1.output,1,self.in_filters[3],self.filter_lengths[3],pool=2.,param_names = ["W4",'b4'],distribution=False)
...
if __name__ == "__main__":
	...
	for i in range(iterations):
		...
		#el,em,es,pri = net.prior_debug(x_train)
		#out = net.output(x_train)
		#print out-x_train
		#compare = 0.5* np.sum(1 + 2*es - em**2 - np.exp(2*es),axis=1)
		#pdb.set_trace()
		...
		print "ITERATION",i
		#if i==10:	
		 	#net.learning_rate.set_value(np.float32(net.learning_rate.get_value()))
		# if i%3==0:	
		# 	plt.clf()
